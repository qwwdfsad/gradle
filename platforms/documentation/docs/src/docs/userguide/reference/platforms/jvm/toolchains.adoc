...
When resolving a toolchain for a specific Java language version, Gradle first checks each installation's `release` file.
Installations declaring a different Java major version are excluded without launching their JVM.
Matching installations, and installations with missing or unrecognized release metadata, are still probed before applying the usual toolchain selection rules.
The `javaToolchains` task continues to inspect all detected installations.
